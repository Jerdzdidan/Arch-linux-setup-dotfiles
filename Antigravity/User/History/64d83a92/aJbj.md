...
## 1. Fix Critical Bugs
- [ ] Fix `UserController::store()` — `user_type` is overridden to `'ADMIN'` on line 66, ignoring the validated input
- [ ] Fix `GoogleAuthController` — uses `name` instead of `first_name`/`last_name` and redirects to non-existent `dashboard`
- [ ] Add `google_id` to User model `$fillable` array
- [ ] Fix `SimulationAttempt::isPassed()` — division by zero when `total_scenarios` is 0
